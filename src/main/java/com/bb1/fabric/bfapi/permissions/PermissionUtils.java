...
public final class PermissionUtils {
	...
	public static final boolean canHavePermission(Field<Entity> field, String node) {
		...
		for (Entry<Identifier, IPermissionDatabase> entry : PERMISSION_DATABASES.getEntries()) {
	...
	public static final void setPermissionExtension(Field<Entity> field, String node, int level, @Deprecated(forRemoval = true) boolean toAll) {
	...
	public static final void givePermission(Field<Entity> field, String node, @Deprecated(forRemoval = true) boolean toAll) {
		if (!canHavePermission(field, node)) { return; }
		final Permission permission = getPermission(node);
		for (Entry<Identifier, IPermissionDatabase> entry : PERMISSION_DATABASES.getEntries()) {
	...
	public static final void takePermission(Field<Entity> field, String node, @Deprecated(forRemoval = true) boolean toAll) {
		final Permission permission = getPermission(node);
		for (Entry<Identifier, IPermissionDatabase> entry : PERMISSION_DATABASES.getEntries()) {
	...
	public static final @Nullable Permission getPermission(String node) {
		for (Entry<Identifier, Permission> entry : BFAPIRegistry.PERMISSIONS.getEntries()) {
